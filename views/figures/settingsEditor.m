function varargout = settingsEditor(varargin)
    % SETTINGSEDITOR M-file for settingsEditor.fig
    % For use with PASimpleSettings and PASettings classes
    
    % Edit the above text to modify the response to help settingsEditor
    
    % Last Modified by GUIDE v2.5 07-Aug-2019 16:51:07
    
    % Begin initialization code - DO NOT EDIT
    gui_Singleton = 1;
    gui_State = struct('gui_Name',       mfilename, ...
        'gui_Singleton',  gui_Singleton, ...
        'gui_OpeningFcn', @settingsEditor_OpeningFcn, ...
        'gui_OutputFcn',  @settingsEditor_OutputFcn, ...
        'gui_LayoutFcn',  [] , ...
        'gui_Callback',   []);
    ...
    % --- Executes just before settingsEditor is made visible.
function settingsEditor_OpeningFcn(hObject, eventdata, handles, varargin)
    % This function has no output args, see OutputFcn.
    % hObject    handle to figure
    % eventdata  reserved - to be defined in a future version of MATLAB
    % handles    structure with handles and user data (see GUIDATA)
    % varargin   command line arguments to settingsEditor (see VARARGIN)
    ...
    % Choose default command line output for settingsEditor
    handles.output = [];
    
    % Update handles structure
    guidata(hObject, handles);
    
    % UIWAIT makes settingsEditor wait for user response (see UIRESUME)
...
function varargout = settingsEditor_OutputFcn(hObject, eventdata, handles)
