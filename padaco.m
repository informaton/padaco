...
% Last Modified by GUIDE v2.5 12-Jun-2014 17:15:25
...
figure_padaco_CloseRequestFcn(gcbf,eventdata,handles);


% --------------------------------------------------------------------
% --- Executes when user attempts to close figure_padaco.
function figure_padaco_CloseRequestFcn(hObject, eventdata, handles)
% hObject    handle to figure_padaco (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
try
    ...
    controller = handles.user.controller;
    if(isa(controller,'PAController'))
        controller.close();
    end
    delete(hObject);
catch ME
