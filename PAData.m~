...
%        sampleRate;
   ...
   methods
       ...
       function dat = getStruct(obj,choice)
           ...
           switch(choice)
               ...
               case 'displayoffset'
                   dat = obj.getCurrentOffsetStruct();
               case 'all'
           ...
           if(exist(fullCountFilename,'file'))
               ...
               if(fid>0)
                   try
                       ...
                       dateTimeNumber = datenum(dateTime,'mm/dd/yyyy,HH:MM:SS');   
                       
                       samplesFound = numel(dateTimeNumber);
                       
                       startDateTimeNum = datenum(strcat(obj.startDate,',',obj.startTime),'mm/dd/yyyy,HH:MM:SS');
                       stopDateTimeNum = dateTimeNumber(end);
                       dateTimeDelta = datenum(0,0,0,0,0,obj.epochPeriodSec);
                       obj.dateTimeNum = startDateTimeNum:dateTimeDelta:stopDateTimeNum;
                       
                       obj.durSamples = numel(obj.dateTimeNum);
                       
                       numMissing = obj.durSamples - samplesFound;
                       missingValue = -1;
                                              
                       if(obj.durSamples==samplesFound)
                           fprintf('%d rows loaded from %s\n',samplesFound,fullCountFilename);  
                           dataCell = tmpDataCell(3:end); 
                           ...
                           %originally found.  
                           dataCell =  repmat({repmat(missingValue,obj.durationSamples(),1)},1,size(tmpDataCell,2)-2);
                           
                           [C,IA,IB] = intersect(allDateTimeNum,fileDateTimeNum);                           
                           
                           dataCell(IB) = tmpDataCell;
                           for c=1:numel(dataCell)
                               dataCell{c}(IB) = tmpDataCell{c}(IB);
                           end
                           
                           if(numMissing>0)
                               fprintf('%d rows loaded from %s.  However %u rows were expected.  %u missing samples are being filled in as %f.\n',samplesFound,fullCountFilename,numMissing,missingValue);
                           else
                               fprintf('%d rows loaded from %s.  However %u rows were expected.  %u missing samples are being filled in as %f.\n',samplesFound,fullCountFilename,numMissing,missingValue);
                           end
                       end
                       ...
                       if(obj.epochPeriodSec>0)
                           obj.durationSec = samplesFound*obj.epochPeriodSec;
                       else
   ...
   methods (Access = private)
       ...
       %> data being displayed.  Values are stored in .position child field
       %> @param Instance of PAData.
       %> @retval tsStruct A struct of [x,y,z] starting location of each data field.  The fields
       %> include:
       %> - accelRaw.x
       %> - accelRaw.y
       %> - accelRaw.z
